...
jQuery(document).ready(
		function() {
			// 左侧导航栏按钮点击后样式
			jQuery(".l_l_nav li a").click(
					function() {
						jQuery(this).css('color', '#09a25f').parent()
								.siblings().children().css('color', '#5a5858');
			...
			// 弹出蒙版
			jQuery(".l_hr_innerl .btn #submit").click(
					function() {
						jQuery("#mask").css('display', 'block');
						jQuery(".promot").css('display', 'block');
						var bodyT = (jQuery(document.body).height() - jQuery(
								".promot").height()) / 2;
						var bodyW = (jQuery(document.body).width() - jQuery(
								".promot").width()) / 2;
